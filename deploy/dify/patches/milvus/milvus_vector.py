...
class MilvusVector(BaseVector):
    ...
    def create(self, texts: list[Document], embeddings: list[list[float]], **kwargs):
        ...
        metadatas = [d.metadata if d.metadata is not None else {} for d in filtered_texts]
        self.create_collection(filtered_embeddings, metadatas, index_params)
        self.add_texts(filtered_texts, filtered_embeddings)

    def add_texts(self, documents: list[Document], embeddings: list[list[float]], **kwargs):
        """
        Add texts and their embeddings to the collection.
        """
        # Check if documents or embeddings is empty
        if not documents or not embeddings:
            logger.warning("No documents or embeddings provided, skipping add_texts")
            return []
        ...
        if not filtered_documents:
            logger.warning("No valid embeddings found, skipping add_texts")
            return []
        ...
        for i in range(len(filtered_documents)):
            insert_dict = {
                ...
                Field.CONTENT_KEY: filtered_documents[i].page_content,
                Field.VECTOR: filtered_embeddings[i],
                Field.METADATA_KEY: filtered_documents[i].metadata,
            }
    ...
    def create_collection(
        ...
        """
        # Check if embeddings is empty or contains None values
        if not embeddings:
            logger.warning("No embeddings provided, skipping collection creation")
            return
        ...
        with redis_client.lock(lock_name, timeout=20):
            ...
            if not self._client.has_collection(self._collection_name):
                ...
                fields.append(FieldSchema(Field.VECTOR, infer_dtype_bydata(filtered_embeddings[0]), dim=dim))
