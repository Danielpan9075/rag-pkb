...
class Vector:
    ...
    def create(self, texts: list | None = None, **kwargs):
        if texts:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                if filtered_batch:
                    self._vector_processor.create(texts=filtered_batch, embeddings=filtered_embeddings, **kwargs)
    ...
    def create_multimodal(self, file_documents: list | None = None, **kwargs):
        if file_documents:
            ...
            for i in range(0, len(file_documents), batch_size):
                ...
                filtered_batch = []
                filtered_embeddings = []
                for doc, emb in zip(real_batch, batch_embeddings):
                    if emb is not None:
                        filtered_batch.append(doc)
                        filtered_embeddings.append(emb)
    ...
    def add_texts(self, documents: list[Document], **kwargs):
        ...
        if filtered_documents:
            self._vector_processor.create(texts=filtered_documents, embeddings=filtered_embeddings, **kwargs)
